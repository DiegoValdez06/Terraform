...
resource "aws_security_group" "SG-ServerWeb" {
...
resource "aws_lb" "ServerWeb-server" {
  name = "ServerWeb-server"
  internal = false
  ip_address_type = "ipv4"
  load_balancer_type = "application"
  subnets = [aws_subnet.SUBNET-PUBLIC.id, aws_subnet.SUBNET-PUBLIC-B.id]

  security_groups = [aws_security_group.SG-ServerWeb.id]
...
resource "aws_lb_listener" "LISTENER-WEB-server" {
  load_balancer_arn = aws_lb.ServerWeb-server.arn
  port = "80" 
  protocol = "HTTP"
...
resource "aws_lb_target_group" "TARGET-GROUP-server" {
  name = "TARGET-GROUP-server"
  port = 80
  protocol = "HTTP"
  vpc_id = aws_vpc.VPC-server.id  
}
...
resource "aws_internet_gateway" "INTERNET-GW-server" {
    vpc_id = aws_vpc.VPC-server.id  
}
...
resource "aws_eip" "EIP-LINUX" {
    ...
    depends_on = [
      aws_internet_gateway.INTERNET-GW-server
    ]  
}
...
resource "aws_route_table" "ROUTES-PUBLIC" {
    ...
    route {
        ...
        gateway_id = aws_internet_gateway.INTERNET-GW-server.id
    }
...
resource "aws_nat_gateway" "NAT-GW-server" {
...
resource "aws_route_table" "ROUTES-PRIVATE-A" {
      ...
      route {
          cidr_block = "0.0.0.0/0"
          nat_gateway_id = aws_nat_gateway.NAT-GW-server.id
      }
